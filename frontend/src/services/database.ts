...
import { User, Course, Request, FeeReceipt, UserCourse, Notification } from '../db/models';
...
export const userService = {
  ...
  async createUser(user: Omit<User, 'id' | 'createdAt' | 'updatedAt'>): Promise<User> {
    ...
    const [result] = await pool.execute(
      ...
      [id, user.name, user.email, user.password, user.rollNo, user.department, user.semester, user.mobileNumber, user.role, user.position, now, now]
    );
...
export const courseService = {
  ...
  async createCourse(course: Omit<Course, 'id' | 'createdAt' | 'updatedAt'>): Promise<Course> {
    ...
    await pool.execute(
      ...
      [id, course.name, course.code, course.department, course.semester, course.credits, course.isElective, course.category, now, now]
    );
...
export const requestService = {
  ...
  async createRequest(request: Omit<Request, 'id' | 'createdAt' | 'updatedAt'>): Promise<Request> {
    const id = uuidv4();
    const now = new Date();
    await pool.execute(
      'INSERT INTO requests (id, userId, type, status, details, holdStartDate, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      [id, request.userId, request.type, request.status, JSON.stringify(request.details), request.holdStartDate, now, now]
    );
    return { ...request, id, createdAt: now, updatedAt: now };
  },

  async updateRequestStatus(id: string, status: 'pending' | 'approved' | 'rejected'): Promise<Request | null> {
    const now = new Date();
    await pool.execute(
...
export const notificationService = {
  ...
  async createNotification(notification: Omit<Notification, 'id' | 'createdAt'>): Promise<Notification> {
    ...
    await pool.execute(
      ...
      [id, notification.title, notification.description, notification.department, notification.deadline, JSON.stringify(notification.readBy || []), now]
    );
...
export const feeReceiptService = {
  async createFeeReceipt(receipt: Omit<FeeReceipt, 'id' | 'createdAt' | 'updatedAt'>): Promise<FeeReceipt> {
    const id = uuidv4();
    const now = new Date();
    await pool.execute(
...
export const userCourseService = {
  async createUserCourse(userCourse: Omit<UserCourse, 'id' | 'createdAt' | 'updatedAt'>): Promise<UserCourse> {
    const id = uuidv4();
    const now = new Date();
    await pool.execute(
