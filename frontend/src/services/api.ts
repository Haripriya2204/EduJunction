...
export const studentService = {
  ...
  getFeeReceiptStatus: async (): Promise<{ status: string, semester: string | null }> => {
    const user = authService.getCurrentUser();
    if (!user) {
      throw new Error('User not authenticated');
    }
    try {
      ...
      return receipt ? { status: receipt.status, semester: receipt.semester } : { status: 'not_uploaded', semester: null };
    } catch (error) {
  ...
  uploadFeeReceipt: async (file: File, semester: string, paymentMode: string, transactionNumber?: string, bankName?: string): Promise<void> => {
    const user = authService.getCurrentUser();
    if (!user) {
      throw new Error('User not authenticated');
    }
  ...
  submitGatePassRequest: async (reason: string, date: string): Promise<void> => {
    const user = authService.getCurrentUser();
    if (!user) {
      throw new Error('User not authenticated');
    }
  ...
  getRequests: async (): Promise<Request[]> => {
    ...
    const response = await fetch('/api/requests/my', { headers: { ...getAuthHeaders() } });
    if (!response.ok) {
  ...
  getNotifications: async (): Promise<Notification[]> => {
    ...
    const response = await fetch(`/api/notifications?department=${encodeURIComponent(user.department)}`, { headers: { ...getAuthHeaders() } });
    if (!response.ok) {
...
export const adminService = {
  getRequestsByStatus: async (status: 'pending' | 'approved' | 'rejected' | 'on_hold'): Promise<Request[]> => {
    const response = await fetch(`/api/requests/department?status=${status}`, { headers: { ...getAuthHeaders() } });
    if (!response.ok) {
      throw new Error('Failed to fetch requests');
    }
  ...
  getAllRequests: async (): Promise<Request[]> => {
    const response = await fetch('/api/requests/department', { headers: { ...getAuthHeaders() } });
    if (!response.ok) {
      throw new Error('Failed to fetch requests');
    }
    return response.json();
  ...
  approveFeeReceipt: async (userId: string, approve: boolean): Promise<void> => {
    // Simulate API delay
    await new Promise(resolve => setTimeout(resolve, 800));
    ...
    const feeReceipts: FeeReceipt[] = JSON.parse(localStorage.getItem('feeReceipts') || '[]');
  ...
  updateRequestStatus: async (id: string, status: 'approved' | 'rejected' | 'on_hold'): Promise<Request> => {
